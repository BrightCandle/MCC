...
switch _mode do {
	case "onLoad": 
	{
		...
		['switchTab',[_display,_tab]] call MCC_RscAttributeOwners;

		{
			_ctrl = _display displayctrl _x;
			_ctrl ctrladdeventhandler [
				"buttonclick",
				format ["with uinamespace do {['switchTab',[ctrlparent (_this select 0),%1]] call MCC_RscAttributeOwners;};",_foreachindex]
			];
		} foreach _idcTabs;
		...
		_playerside = player call bis_fnc_objectside;
		{
			...
			_ctrl ctrladdeventhandler ["buttonclick","with uinamespace do {['selectSide',[_this select 0,0.1]] call MCC_RscAttributeOwners;};"];
			if (_side in _selected) then {
				['selectSide',[_ctrl,0]] call MCC_RscAttributeOwners;
			};
		...
		{
			_grp = group _x;
			_grpSideIcon = _sideIcons select ((side _grp) call bis_fnc_sideID);
