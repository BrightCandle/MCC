...
if (_shape == "ICON") then {
  ...
  if ([_pos,_center] call GAIA_fnc_isSamePosition) then {
    _return = true;
  };
...
// Markers that have an area.
  ...
  if (_shape in ["RECTANGLE","SQUARE"]) then {
    private ["_corners"];
    _corners = _area call GAIA_fnc_getMarkerVertices;
    ...
    if (_dir % 90 != 0) then {
      ...
      for "_i" from 0 to (count _corners - 1) do {
        ...
        _posNew = [[_shiftedX,_shiftedY],_dir] call GAIA_fnc_rotatePosition;
    ...
    if (_shape == "CIRCLE") then {
      _return = [_pos,_area] call GAIA_fnc_isInCircle;
    } else {
      _return = [_pos,_area] call GAIA_fnc_isInEllipse;
    };
  };
};
