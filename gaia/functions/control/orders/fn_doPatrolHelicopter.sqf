	...
	if (  ((_group) getVariable ["GAIA_LandingSpot",[0,0,0]] distance [0,0,0]==0)  ) then
	{
		...
		if (isTouchingGround (assignedvehicle leader _group)) then
		...
		else
			{
				...
				_landpos =  [([_zone,"ARM_HILLS_FLAT",(side _group)] call  GAIA_fnc_calculateOptimalPosition), 0,100, 12, 0, 60 * (pi / 180), 0] call BIS_fnc_findSafePos;
	...
	then
	{
		...
		//Go somewhere
			...
			if (((_group getVariable  ["GAIA_zone_intend",[]]) select 1)=="NOFOLLOW") then
				{_pos= ([_zone ,"AIR",(side _group)] call  GAIA_fnc_calculateOptimalPosition);}
			else
				{_pos= ([(_zns  call BIS_fnc_selectRandom ) ,"AIR",(side _group)] call  GAIA_fnc_calculateOptimalPosition);};
