...
_shape = _area call GAIA_fnc_getMarkerShape;
...

{
	...
	if (_shape in ["SQUARE","RECTANGLE"]) then
		...
		{
				 _pos = _area call GAIA_fnc_randomPositionInRectangleMarker;
		}

		else
		{
	      _pos = _area call GAIA_fnc_randomPositionInEllipseMarker;
		};

		_SRange = 90;

		//Scan the terrain
		_topArr = [_pos,1] call GAIA_fnc_analyzeTerrain;
		...
		if (!isonroad _pos) then
		{
			if ((_goal == "VEH_HILLS_ROAD_FLAT" or _goal == "VEH_HILLS_FOREST_FLAT" or _goal == "ARM_HILLS_FLAT" ) and (_found>_best ) ) then
				//Make sure the vehicle actualy fits (NOT within 3 meter of objects)
				{
						...
						if (count(_pos)==0 or !([_pos,_side] call GAIA_fnc_isBlacklisted)) then
							{_pos = _oldpos};
				};
		};
	  ...
	  if ([_pos,_side] call GAIA_fnc_isBlacklisted) exitwith
				{
		  		_found= -10000;
				};
		};
